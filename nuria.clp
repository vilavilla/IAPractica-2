...
(defmodule abstraccion (import getInput ?ALL) (export ?ALL) )

(defmodule asociacionHeuristica (import abstraccion ?ALL) (export ?ALL) )

(defmodule showResultado (import asociacionHeuristica ?ALL) (export ?ALL) )
...
(defrule abstraccion::crear-instancia-visita
    ?datos <- (datosVisita (Npersonas_visita ?np)
                          (Conocimiento_visita ?cv)
                          (Ndias_visita ?nd)
                          (Duracion_visita ?dv)
                          (Hay_peques_visita ?hp)
                          (Tematica $?t)
                          (Epoca $?e)
                          (Estilo $?es)
                          (Pintor $?p))
    =>
    (make-instance [nueva-visita] of Visita
        (Npersonas_visita ?np)
        (Conocimiento_visita ?cv) 
        (Ndias_visita ?nd)
        (Duracion_visita ?dv)
        (Hay_peques_visita ?hp)
        (vista_tiene_preferencia $?t $?e $?es $?p))
